#!/usr/bin/env python3
"""
Smoke test for dbus_next (0.2.3): confirms the library imports, a session
...
async def main():
    try:
        ...
        fail(
            f"dbus_next not importable ({e}). Install with: "
    ...
    try:
        notif_id = await notifications.call_notify(
            "dbus-next-smoke-test",
            0,
            "dialog-information",
            "dbus_next smoke test",
            "If you see this, dbus_next + your notification daemon both work.",
            [],
            {"urgency": Variant("y", 1)},
            4000,
        )
    ...
    print("[OK] dbus_next environment looks healthy.")
